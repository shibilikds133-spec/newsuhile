...
(async () => {
  const styleName = process.argv[2] || 'none';
  const styleCode = {
    consolas: 'amount.style.fontFamily="Consolas, monospace";',
    arial: 'amount.style.fontFamily="Arial, sans-serif";',
    table: 'row.style.display="table-cell";row.style.width="216px";row.style.height="60px";row.style.verticalAlign="middle";row.style.textAlign="center";amount.style.display="inline";rupee.style.display="inline";',
    absolute: 'row.style.position="relative";const wrap=document.createElement("span");wrap.style.cssText="position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);white-space:nowrap;display:inline-flex;align-items:center;gap:4px;";row.insertBefore(wrap,rupee);wrap.appendChild(rupee);wrap.appendChild(amount);',
    none: '',
  }[styleName];
  const base = path.resolve(__dirname, '..', '..'), dir = __dirname;
  ...
  try {
    ...
    await page.type('input[name="manualReceiptNo"]', `AUD-${styleName}`);
    await page.type('input[name="amount"]', '25000');
    await page.click('button[type="submit"]');
    await page.waitForSelector('#receipt-element', { visible: true, timeout: 20000 });
    await wait(500);
    const previewPath = path.join(dir, `one-${styleName}-preview.png`);
    const pngPath = path.join(dir, `one-${styleName}-png.png`);
    await page.evaluate(code => {
      ...
      Function('row', 'amount', 'rupee', code)(row, amount, rupee);
    }, styleCode);
    ...
    const meta = await page.evaluate(async () => {
      ...
      const rupee = amount.previousElementSibling;
      const row = amount.parentElement;
      const er = el.getBoundingClientRect(), rr = row.getBoundingClientRect(), ar = amount.getBoundingClientRect(), ur = rupee.getBoundingClientRect();
      const meta = { row: { left: rr.left - er.left, top: rr.top - er.top, right: rr.right - er.left, bottom: rr.bottom - er.top }, union: { left: Math.min(ar.left, ur.left) - er.left, top: Math.min(ar.top, ur.top) - er.top, right: Math.max(ar.right, ur.right) - er.left, bottom: Math.max(ar.bottom, ur.bottom) - er.top } };
      ...
      meta.data = c.toDataURL('image/png');
      el.style.width = ow; el.style.transform = ot; el.style.position = op;
      return meta;
    });
    fs.writeFileSync(pngPath, meta.data.replace(/^data:image\/png;base64,/, ''), 'base64');
    const roi = { x0: meta.union.left - 4, y0: meta.union.top - 4, x1: meta.union.right + 4, y1: meta.union.bottom + 8 };
    const preview = await bounds(previewPath, 1, roi), png = await bounds(pngPath, 2, roi);
    const result = { styleName, meta: { row: meta.row, union: meta.union }, preview, png, diff: { top: +(png.top - preview.top).toFixed(3), bottom: +(png.bottom - preview.bottom).toFixed(3), height: +(png.height - preview.height).toFixed(3), previewTopGap: +(preview.top - meta.row.top).toFixed(3), previewBottomGap: +(meta.row.bottom - preview.bottom).toFixed(3), pngTopGap: +(png.top - meta.row.top).toFixed(3), pngBottomGap: +(meta.row.bottom - png.bottom).toFixed(3) } };
    fs.writeFileSync(path.join(dir, `one-${styleName}-result.json`), JSON.stringify(result, null, 2));
